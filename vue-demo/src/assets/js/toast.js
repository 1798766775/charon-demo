...
Toast.install = function (Vue, options) {
  ...
  var opt = {
    ...
    duration: '3500',
    wordWrap: false,
    defaultTipsType : 'success'
  };
  for (var property in options) {
    opt[property] = options[property];
  }

  Vue.prototype.$toast = function (tips,tipType, type) {

    var curType = type ? type : opt.defaultType;
    tipType = tipType ? tipType : opt.defaultTipsType;
    var wordWrap = opt.wordWrap ? 'lx-word-wrap' : '';
    var style = opt.width ? 'style="width: ' + opt.width + '"' : '';
    var tmp = '<div v-show="show" :class="type" class="lx-toast ' + wordWrap + '" ' + style + '><span></span>{{tip}}</div>';
    ...
    toastVM.type = 'lx-toast-' + curType + "-" + tipType;
    toastVM.tip = tips;
